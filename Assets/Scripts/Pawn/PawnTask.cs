...
public static class PawnTask
{
    ...
    public static void LineUpPawnsOnTarget(List<PawnAI> pawnList, Vector3 centerPoint)
    {
        ...
        {
            PawnAI pawn = pawnList[pawnIndex];

            int row = pawnIndex / cols;
            int col = pawnIndex % cols;

            Vector3 offsetFromCenter = new Vector3((row - halfRows) * pawn.Offset, 0, (col - halfCols) * pawn.Offset);
            Vector3 posInLine = centerPoint + offsetFromCenter;

            pawn.MoveToTarget(posInLine);
            pawn.TargetPosition = posInLine;
            pawn.StopFollowing();
        }
    }

    public static void CircleTarget(List<PawnAI> pawnList, Vector3 centerPoint)
    {
        for (int pawnIndex = 0; pawnIndex < pawnList.Count; pawnIndex++)
        {
            PawnAI pawn = pawnList[pawnIndex];
            ...
            pawn.TargetPosition = posInCircle;
            pawn.StopFollowing();
        }
    }
}
